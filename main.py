...
if __name__ == '__main__':
    # 获取设备列表
    devs = get_devices(save=True)
    print(devs)
    time.sleep(2)

    # 获取房间列表
    rooms = get_rooms(save=True)
    print(rooms)
    time.sleep(2)

    # 获取场景列表
    scenes = get_scenes(save=True, room_idx=0)
    print(scenes)
    time.sleep(2)
    ...
    for dev in devs['result']['list']:
        if dev.get('model') == 'yeelink.light.lamp4':
            ...
            break
    if myLight:
        isOpen = get_dev_att([{"did": myLight, "siid": 2, "piid": 1}])  # 获取台灯的开关状态
        if isOpen.get('code') == 0:
            if isOpen['result'][0]['value']:
                ...
                if status.get('code') == 0:
                    ...
                    print('获取台灯亮度和色温失败')
                set_dev_att([{"did": myLight, "siid": 2, "piid": 1, "value": False}])  # 关闭台灯
            else:
                print('台灯已关闭')
        else:
            print('获取台灯开关状态失败')
